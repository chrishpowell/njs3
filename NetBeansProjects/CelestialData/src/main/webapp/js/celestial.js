...
//**************************************************************************** [FE Config]
// ROOT: Show it all, with the given config, otherwise with default settings
// Long beast (870 lines...)
...
// See also getData()
  function load()
  {
    ...
    // Get the Constellation names or designation
    ...
    // Get the Constellation boundaries
    ...
    // Get the Deep space objects (DSOs)
  ...
  //*************************************************************************** [FE]
  // Redraw all objects on canvas.  See also drawOutline()
  function redraw()
  {  
...
//    drawOutline(true);
  ...
  // Load
  // ----
  // Calls:  getData():Load data ->
  //               redraw():Draw() [rotate,setCenter,zoom etc.] ->
  //                    drawOutline():? [rotate,rotate...??]  ->
  //                        d3.geo.projection(<some function>)
  load();
}; //==================== End Celestial.display() ==============================
...
Celestial.projection = function(projection)
{
  ...
  return d3.geo.projection(forward);  // OLD d3?
};
...
//*****************************************************************************
// *** Check numeric field (Hr/Min/sec) ***
// Validate with: <input id="num" type="number">.
// Validate whole text [regex: new RegExp('^[0-9]+$');] after (for non-conforming browsers)
// Num range: https://www.regular-expressions.info/numericranges.html
...
function geo(cfg)
{
  ...
  //Here-button if supported  ************** NOT Needed *********************
...
//  col.append("input").attr("type","date").attr("id","dtdate").attr("value","1900-12-01").on("input", function(){alert(this.value); dtdate = this.value;});
//  col.append("input").attr("type","time").attr("id","datetime").attr("value","23:59").on("input", function(){dtdate+="T"+this.value;alert(dateFormat(this.value));go();});
    col.append("div").attr("id","tz").html("Your timezone is: "+Intl.DateTimeFormat().resolvedOptions().timeZone);
