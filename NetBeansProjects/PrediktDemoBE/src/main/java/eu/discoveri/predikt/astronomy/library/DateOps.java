 ...
 * <P>
 * This code supports conversions between JD and two calendrical systems: Julian
 * and Gregorian. Comments pertaining to specific calendars are found near the
 * code for those calendars.
 * <P>
 * [mhuss: The original C code, which these comments are paraphrased from,
 * supported other calendars - these may be supported here in the future, but
 * for now we're pretty Gregorian.]
 * <P>
 * For each calendar, there is a <TT>(cal)DateConversionData</TT>" class, used
 * only within this code module. This class takes a particular year number, and
 * computes the JD corresponding to "new years day" (first day of the first
 * month) in that calendar in that year. It also figures out the number of days
 * in each month of that year, returned in the array month_data[]. There can be
 * up to 13 months, because the Hebrew calendar (and some others that may
 * someday be added) can include an "intercalary month". If a month doesn't
 * exist, then the month_data[] entry for it will be zero (thus, in the
 * Gregorian and Julian and Islamic calendars, month_data[12] is always zero,
 * since these calendars have only 12 months.)
 * <P>
 * [mhuss: The code here only supports Gregorian and Julian, so the class is
 * just called <TT>DateConversionData</TT>. When I get around to doing the rest,
 * <TT>DateConversionData</TT> will be the interface or base class used by all.]
 * <P>
 * The next level up is the <TT>getCalendarData()</TT> function, which can get
 * the JD of New Years Day and the array of months for any given year for any
 * (supported) calendar. Above this point, all calendars can be treated in a
 * common way; one is shielded from the oddities of individual calendrical
 * systems.
 * <P>
 * Finally, at the top level, we reach the only functionality that is exported
 * for the rest of the world to use: <TT>dmyToDay()</TT> and
 * <TT>dayToDmy()</TT>. The first (and its variations) takes a day, month, year,
 * and optionally, a calendar system. It calls
 * <TT>getCalendarData()</TT> for the given year, adds in the days in the months
 * intervening New Years Day and the desired month, adds in the day of the
 * month, and returns the resulting Julian Day.
 * <P>
 * dayToDmy() reverses this process. It finds an "approximate" year
 * corresponding to an input JD, and calls <TT>getCalendarData()</TT>
 * for that year. By adding all the monthData[] values for that year, it can
 * also find the JD for the <B>end</B> of that year; if the input JD is outside
 * that range, it may have to back up a year or add in a year. Once it finds the
 * year where "JD of New Years Day < JD < JD of New Years Eve", it's a simple
 * matter to track down which month and day of the month corresponds to the
 * input JD.
 */
public class DateOps {
    ...
    * month and day of month we're in.
     ...
     * a given year.
     * <P>
     * The chosen start day is the first Sunday in April, and therefore this
     * function may not be applicable outside the U.S.
     *
     ...
     * given year.
     * <P>
     * This chosen day is the last Sunday in October, and therefore this
     * function may not be applicable outside the U.S.
     *
     * @param year The year of interest
     *
