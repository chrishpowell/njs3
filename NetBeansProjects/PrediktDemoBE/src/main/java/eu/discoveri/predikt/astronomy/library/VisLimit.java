 ...
 * on Brad Schaefer's article and code on pages 57-60, May 1998 _Sky &
 * Telescope_, "To the Visual Limits".
 * <P>
 * <I>[Bill Gray's Comments:]</I><BR>
 * The computations for sky brightness and limiting magnitude can be logically
 * broken up into several pieces. Some computations depend on things that are
 * constant for a given observing site and time: the lunar and solar zenith
 * distances, the air masses to those objects, the temperature and relative
 * humidity, and so forth. For use in Guide, I expect to compute brightness at
 * many points in the sky, while all these other values hold constant. So my
 * first step (after putting lat/lon and these other data into an instance of
 * VisLimitAngularBrightnessData) is to call the setBrightnessParams() function.
 * This function does a lot of "setup work", figuring out the absorption per
 * unit air mass at various wavelengths from various causes (gas, aerosol,
 * ozone), the number of air masses to the sun and moon, and so forth.
 * <P>
 * Once you've done all this, you can call computeSkyBrightness() for any point
 * in the sky. You do need to provide the zenith angle, and the angular distance
 * of that point from the moon and sun. The brightnesses are available via the
 * brightness() functions. The 'mask' value can be used to specify which of the
 * five bands is to be computed. (For example, if I use this to make a realistic
 * sky background, I may just concern myself with the V band... maybe with B and
 * R if I want to attempt a colored sky. In either case, computing all five
 * bands would be excessive.)
 * <P>
 ...
 /* scattered?   2240 */
  ...
  private double /*air_mass,*/ brightness[] = {0D, 0D, 0D, 0D, 0D};
    ...
    private static double computeFFactor(double objDist) {
        ...
        rval += 229086. * (1.06 + cosDist * cosDist);
        /* polarization term? */
